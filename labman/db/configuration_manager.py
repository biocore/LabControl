...
class ConfigurationManager(object):
    ...
    """
    @staticmethod
    def create(config_fp, test_env, certificate_file, key_file, db_host,
               db_port, db_name, db_user, db_password, db_admin_user,
    ...
    def _get_main(self, config):
        ...
        if not self.certificate_file:
            self.certificate_file = join(self.install_dir, 'gui',
                                         'support_files', 'server.crt')

        self.key_file = config.get('main', 'KEY_FILE')
        if not self.key_file:
            self.key_file = join(self.install_dir, 'gui', 'support_files',
                                 'server.key')
...
LOG_DIR=%(logdir)s
CERTIFICATE_FILE=%(certificate_file)s
KEY_FILE=%(key_file)s
